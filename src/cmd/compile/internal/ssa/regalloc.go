...
func (s *regAllocState) init(f *Func) {
	...
	case s.ZeroReg:
		if f.Config.hasZeroReg {
			s.f.Fatalf("no zero register found")
		}
	}
	...
	if s.f.Config.hasZeroReg {
		s.allocatable &^= 1 << s.ZeroReg
	}
	...
	for _, b := range s.visitOrder {
		...
		for i := len(b.Values) - 1; i >= 0; i-- {
			...
			if opcodeTable[v.Op].call {
				...
				if s.zero != 0 && s.values[s.zero].uses != nil {
					regValLiveSet.add(s.zero)
				}
			}
			for _, a := range v.Args {
				if !s.values[a.ID].needReg {
		...
		for idx, v := range oldSched {
			...
			if v.Op == OpHardwiredZero && s.f.Config.hasZeroReg { // use hardware zero register
				s.assignReg(s.ZeroReg, v, v)
				b.Values = append(b.Values, v)
				s.advanceUses(v)
				s.zero = v.ID
				continue
...
func (v *Value) rematerializeable() bool {
	...
	for _, a := range v.Args {
		...
		if a.Op != OpSP && a.Op != OpSB && a.Op != OpHardwiredZero {
			return false
		}
	}
	return true
